...
const createSpace = async(req, res) =>{
    const {headquarter_id, space_name, space_fee, space_description, rol} = req.body
...
const modifySpace = async(req, res) =>{
    const {space_id, new_headquarter_id, new_space_name, new_space_fee, new_space_description, rol} = req.body
    jwt.verify(req.token, 'secretkey', async(error) =>{
        if(!error){
            if(rol === "A" || rol === "a"){
